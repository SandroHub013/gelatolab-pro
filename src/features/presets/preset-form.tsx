...
export const DEFAULT_PRESET_TARGETS: TargetRow[] = [
  { key: "totalSolids", min: 30, ideal: "38", max: 44 },
  { key: "sugars", min: 14, ideal: "17", max: 20 },
  { key: "fat", min: 6, ideal: "9", max: 13 },
  { key: "pod", min: 15, ideal: "18", max: 22 },
  { key: "pac", min: 20, ideal: "24", max: 28 },
];

export function targetRowsFrom(ranges: TargetRanges): TargetRow[] {
...
export interface PresetFormValues {
  ...
  targetRanges: CalibrationPresetInput["targetRanges"];
}

/**
 * Form condiviso per creazione e modifica dei preset: le due pagine devono
 * applicare le stesse regole di validazione sui range.
 */
export function PresetForm({
  initialName = "",
  initialDescription = "",
  initialFamilies = ["base_latte"],
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    for (const t of targets) {
      ...
      if (ideal !== undefined && (ideal < t.min || ideal > t.max)) {
        setError(`Il target ${TARGET_LABELS[t.key]}: l'ideale deve essere compreso tra min e max.`);
        return;
      }
    ...
    startTransition(async () => {
      try {
        await onSubmit({
          ...
          targetRanges: targetRanges as CalibrationPresetInput["targetRanges"],
        });
      } catch (err) {
